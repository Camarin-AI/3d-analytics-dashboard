...
export async function GET(request: NextRequest) {
  ...
  const route = '/api/weekly-visitors';
  try {
    ...
    if (!from || !to) {
      ...
      logger.error('Weekly Visitors API error: Date range parameters (from, to) are required', {
        from,
        to
      });
    ...
    logger.info('Successfully processed /api/weekly-visitors');
    return NextResponse.json(data);
  } catch (error) {
    logger.error('Weekly Visitors API error:', error);
